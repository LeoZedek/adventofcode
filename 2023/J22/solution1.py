...
for supported_bricks in suporte.values():
	not_good_to_destroyed = False

	for supported_brick in supported_bricks:
		if len(is_supported_by[supported_brick]) <= 1:
			not_good_to_destroyed = True

	if not(not_good_to_destroyed):
		result += 1

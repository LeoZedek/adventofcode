...
class Monkey():
	...
	def update_firts_worry_level_item(self):
		...
		if operation[1] == "+":
			...
			print("error")
		print(new_level)
		#self.items[0] = new_level
		self.items[0] = floor(new_level / 3)
...
for k in range(10):
	...
	for ind_monkey, monkey in enumerate(monkeys):
		...
		while len(monkey.get_items()) > 0:
			...
			monkey.update_firts_worry_level_item()

			if monkey.get_items()[0] % monkey.divisibleIntTest == 0:
				print("coucou {}".format(ind_monkey))

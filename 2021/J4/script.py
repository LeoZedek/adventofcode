...
nombreGrilles = len(grilles)

while not(partieFini):
	nombreTire.append(numbers[indice])

	for i in range(nombreGrilles):
		if estGrilleGagnante(grilles[i]):
			partieFini = True
			indiceGrillesGagnantes = i
	
	indice = indice + 1
	
print(grilles[indiceGrillesGagnantes])

grilleG = grilles[indiceGrillesGagnantes]

resultat = 0

for i in range(n):
	for j in range(p):
		if not(grilleG[i][j] in nombreTire):
			resultat = resultat + int(grilleG[i][j])

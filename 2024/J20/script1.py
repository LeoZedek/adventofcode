...
def get_cheats(row, col, lenght):
	cheats = set()

	for di in range(-lenght, lenght + 1):
		for dj in range(-lenght, lenght + 1):
			if abs(di) + abs(dj) == lenght:
				cheats.add(((row, col), (row + di, col + dj)))

	return cheats

def get_possible_cheat(path, distance, row, col, threshold):
	cheats = get_cheats(row, col, 2)

	result = set()

	for cheat in cheats:
		end_cheat = cheat[1]
		if end_cheat in path and (distance[end_cheat] - distance[(row, col)] - 2 >= threshold):
			result.add(cheat)			
...
def main():
	...
	r = 0
	for (i, j) in path:
		r += len(get_possible_cheat(path, distance, i, j, 100))
